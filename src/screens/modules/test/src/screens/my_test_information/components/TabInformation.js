...
const TabInformation = (props) => {
  const { navigation, type, data, isCurrentTestStatus, idBack, params } = props;
    ...
    if (isCurrentTestStatus === Constant.MY_TEST_STATUS.goingOn) {
      if (data?.password) {
        ...
        const params = {
          ...
          securityCode: null,
          isExtraTest:
            data?.extraTestCount != null && data?.extraTestCount !== 0,
        };
        /**
         * Call api startTest
         */
  ...
  return (
    <>
      ...
      >
        {listInfo.map((item, index) => itemInfor(item, index))}
      </ScrollView>
      <TouchableDebounce
        style={[
          styles.btnEnterExam,

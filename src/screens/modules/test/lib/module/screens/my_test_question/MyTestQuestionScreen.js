...
import { KeyboardAvoidingView, SafeAreaView, ScrollView, View, AppState, BackHandler, Keyboard } from 'react-native';
...
import { doTestMobile, endTestNew, updateAnswer, updateBookmark, updateViolationLog } from "../../services/testregistorusertest.api.js";
import { styles } from "./MyTestQuestionScreen.styles.js";
...
const MyTestQuestionScreen = props => {
   ...
   */
  useEffect(() => {
    const socketUrl = `${storage.getString(Constant.DOMAIN)}${enviroment.apiDomain.testRegistorFrontEndSocketEndpoint}?ruId=${id}`;
   ...
   */
  const onHandlerSubmit = useCallback(async event => {
    setShowModal(false);
    setIsResult(true);
    refPlay?.current?.hidde();

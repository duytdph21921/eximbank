...
const BottomSheetFilterClassRoom = (props) => {
   ...
   * Trạng thái học tập.
   */
  const RederStatusLearn = useCallback(
    () => (
      <View>
        <RenderViewTitle
        ...
        <ItemSwitchDots
          ...
          onPress={() => {
            setLmsClassFilter((prevState) => ({
              ...prevState,
              statusLearn: prevState.statusLearn.map((item, i) =>
   ...
   * Trạng thái bắt buộc.
   */
  const RenderStatusRelation = useCallback(
    () => (
      <View>
        <RenderViewTitle
   ...
   */
  const RenderStatusClass = useCallback(
    () => (
      <View>
        <RenderViewTitle
        ...
        <ItemSwitchDots
          ...
          onPress={() => {
            setLmsClassFilter((prevState) => ({
              ...prevState,
              statusClass: prevState.statusClass.map((item, i) =>
                i === 0 ? { ...item, A: !item.A } : item
              ),
            }));
          }}
   ...
   * Loại tổ chức.
   */
  const RenderClassOrganizationType = useCallback(
    () => (
      <View>
        <RenderViewTitle
        ...
        <ItemSwitchDots
          ...
          onPress={() => {
            setLmsClassFilter((prevState) => ({
              ...prevState,
              classOrganizationType: prevState.classOrganizationType.map(
                (item, i) => (i === 0 ? { ...item, A: !item.A } : item)
              ),
            }));
          }}
        />
  ...
  const onDeleteAction = (key) => {
    let lmsClassFilterNew = { ...lmsClassFilter };
    switch (key) {
      case 'type_of_item':
        // Xoa het
        lmsClassFilterNew = {
  ...
  const transformObject = (obj) => {
    ...
    const newClassOrganizationType = obj.classOrganizationType.flatMap(
      (item) => {
        const result = [];
        if (item.A) {
          result.push(1);
            ...
            <View style={[styles.buttonSheet]} onLayout={onViewLayout}>
              ...
              >
                <TouchableDebounce activeOpacity={1}>
                  ...
                  />
                  <RederStatusLearn />
                  <RenderStatusRelation />
                  <RenderStatusClass />
                  <RenderClassOrganizationType />
                  <TouchableDebounce
                    style={[
                      styles.btnApply,
                      {

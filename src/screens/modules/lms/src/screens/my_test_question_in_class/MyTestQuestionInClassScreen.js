...
import {
  ...
  updateAnswerNew,
  updateBookmark,
} from '../../services/testclassusertest.api';
import { frUserFinishTestNew } from '../../services/lmsclass.api';
import { styles } from './MyTestQuestionInClassScreen.styles';
...
const MyTestQuestionInClassScreen = (props) => {
  ...
  useEffect(() => {
    const listNew = dataMyTest?.dataTest?.filter(
      (item) => item.questionType !== -1
    );
    const listBookmarkNew = dataMyTest?.bookMark?.filter(
      (item) => item.questionType !== -1
    );
  ...
  const onHandlerSubmit = useCallback(async () => {
    ...
    if (response?.status) {
      ...
      if (
        data?.resultDisplayType !==
          Constant.RESULT_DISPLAY_TYPE.showNotDetail &&
        data?.resultDisplayType !== Constant.RESULT_DISPLAY_TYPE.showNotAll
      ) {

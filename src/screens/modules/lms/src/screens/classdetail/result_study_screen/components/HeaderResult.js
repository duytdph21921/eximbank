...
const HeaderResult = (props) => {
  const { onHandleAggregate, dataChart } = props;
  const data = dataChart?.percentLearning ?? 0;
  return (
    <View style={styles.container}>
      <IconHeader width={WIDTH_IMAGE} height={HEIGHT_IMAGE} />
      <View style={styles.viewContent}>
        <View style={styles.viewMark}>
          <CMText i18nKey="text-header-mark" style={styles.textTitleMark} />
          <View style={styles.viewMarkDetail}>
          ...
          <TouchableDebounce style={styles.btnStateStudy}>
            <CMText
        ...
        <View style={styles.viewChart}>
          <ProgressCircle
            ...
            strokeWidth={20}
          />
          <CMText title={`${data}%`} style={styles.percentage} />
        </View>
      </View>
    </View>
  );

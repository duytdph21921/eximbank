...
import { styles } from './SurveyResultScreen.styles';

const SurveyResultScreen = (props) => {
  const { navigation, route } = props;
  const { surveyId, surveyUserId, sourceThank } = route?.params || {};
  ...
  return (
    <SafeAreaView style={styles.container}>
      <IconCompleteSurvey width={150} height={150} />
      <CMText i18nKey="text-complete-survey" style={styles.textComplete} />
      {/* <CMText
        i18nKey={"text-complete-thank-you-survey"}
        style={styles.textCompleteTk}
      /> */}
      {sourceThankHtml && (
        <RenderHtml
      ...
      >
        <CMText i18nKey="text-buton-re-view-exam" style={styles.textFooter} />
      </TouchableDebounce>
    </SafeAreaView>
  );
};

...
import { getBySubjectIdAndUser } from '../../../services/lmsclass.api';
...
const ListClass = (props) => {
  ...
  const renderItemClass = (item, index) => (
    <TouchableDebounce
      style={[
        styles.viewItemClass,
        {
          marginLeft:
            index % 2 !== 0
              ? screenWidth - horizontal(30 * 2) - WIDTH_ITEM * 2
              : 0,
        },
      ]}
    ...
    >
      ...
      <View style={styles.viewClassType}>
        <CMText title={`${item?.classTypeName}`} style={styles.textTypeClass} />
      </View>
    </TouchableDebounce>
  );
  const onPressFilter = () => {
  ...
  const handleLoadMore = () => {
    if (
      ...
      listClass.length < totalClass
    ) {
      setLoadMore(true);
      const newModel = { ...paramsListClass, offset: offset + limit };
  ...
  return (
    <View style={styles.container}>
      <FlatList
        ...
        renderItem={({ item, index }) => renderItemClass(item, index)}

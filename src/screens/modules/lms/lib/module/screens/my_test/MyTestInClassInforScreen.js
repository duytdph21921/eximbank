...
import { updateLoadingAction, updateParamTestInClassInforScreen } from '@store/reducers/globalSlice';
...
import { styles } from "./MyTestInClassInforScreen.styles.js";
import { getTestFormInfo } from "../../services/testclassusertest.api.js";
...
const MyTestInClassInforScreen = props => {
  ...
  const paramsTestInClassInforScreen = useSelector(state => state.global.paramsTestInClassInforScreen);
  ...
  useEffect(() => {
    if (learningId && !classInfo?.isCloseClass) {
      const socketUrl = `${storage.getString(Constant.DOMAIN)}${enviroment.apiDomain.lrsEndpoint}?learningId=${learningId}`;
      connection = new signalR.HubConnectionBuilder().withUrl(socketUrl, {
        accessTokenFactory: async () => storage.getItemString(Constant.KEY_USER_TOKEN),
  ...
  const funcGetTestFormInfo = async () => {
    ...
    if (response?.status && isMounteRef.current) {
      dispatch(updateParamTestInClassInforScreen({
        testFormId: route?.params && route?.params?.callBack ? paramsTestInClassInforScreen?.testFormId : content?.contentOpenId,
        classContentId: route?.params && route?.params?.callBack ? paramsTestInClassInforScreen?.classContentId : content?.id,
        classUserId: route?.params && route?.params?.callBack ? paramsTestInClassInforScreen?.classUserId : classUserId,

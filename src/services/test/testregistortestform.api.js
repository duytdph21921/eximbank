...
export async function getById(id) {
  const urlApi = `${enviroment.apiDomain.testRegistorFrontEndEndpoint}/TestRegistorTestForm/GetById/${id}`;
  const response = await client.get(urlApi);
  return response;
}
/**
...
export async function getByRegistorId(model) {
  const urlApi = `${enviroment.apiDomain.testRegistorFrontEndEndpoint}/TestRegistorTestForm/GetByRegistorId`;
  const response = await client.post(urlApi, model);
  return response;
}

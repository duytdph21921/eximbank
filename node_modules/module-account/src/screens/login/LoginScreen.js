...
import {
  ...
  getConfigImageLogin,
  getByKey,
} from '../../services/authentication/systemsettings.api';
import { updateDevices } from '../../services/hr/hrprofiledevices.api';
import BottomSheetLanguage from '../../component/BottomSheetLanguage';
...
const LoginScreen = ({ navigation }) => {
  ...
  const onHandlerLogin = async (params) => {
    ...
    if (login.fulfilled.match(response)) {
      ...
      if (isRemember === Constant.IS_REMEMBER) {
        ...
        Keychain.setGenericPassword(usename, password)
          .then(() => console.log('Password stored successfully'))
          .catch((error) => console.error('Error storing password:', error));
      } else {
        storage.set(Constant.REMEMBER_ACCOUNT, Constant.IS_NOT_REMEMBER);
        Keychain.resetInternetCredentials(usename, password)
          .then(() => console.log('Password reset stored successfully'))
          .catch((error) => console.error('Error storing password:', error));
      }
  ...
  const onHandelLoginBiometry = () => {
    ...
    if (languageLocal === Constant.LANGUAGE_VN) {
      textNotifi = isSupportedFaceID
        ? PLACEHOLDER.vn.faceIdText
        : PLACEHOLDER.vn.touchIdText;
    } else {
      textNotifi = isSupportedFaceID
        ? PLACEHOLDER.en.faceIdText
        : PLACEHOLDER.en.touchIdText;
    }
  ...
  const getMenuApp = async () => {
    const response = await getByKey(
      'SETTINGS_SYSTEMSETTINGS_SERCURITY_CONFIG_MENU_APP'
    );
    if (response?.status && response?.data) {
      // parse json ra
        ...
        >
          ...
          <TouchableDebounce
            style={[
              ...
              {
                backgroundColor:
                  checkEmptyPassword() && checkEmptyUsename()
                    ? appColor?.base_color
                    : Color.color_border,
              },
            ]}
